<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:66:"C:\xampp\htdocs\houtai\public/../application/admin\view\login.html";i:1534126044;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    ...
    <title><?php echo $admintitle; ?>后台登录</title>
...
<body class="signin"  style="background: url(/static/admin/images/login-background-<?php echo $bgnum; ?>.jpg) no-repeat center fixed;background-size: cover">
<div class="signinpanel">
            ...
            <form id="doLogin" name="doLogin" method="post" action="<?php echo url('doLogin'); ?>">
                ...
                <?php if($verify_type == 1): ?>
                    <div style="margin-bottom:70px">
                        ...
                        <img id="verifyimg" src="<?php echo url('checkVerify'); ?>" onclick="javascript:this.src='<?php echo url('checkVerify'); ?>?tm='+Math.random();" style="float:right;cursor: pointer"/>
                    </div>
                <?php else: ?>
    ...
    <div class="signup-footer">
        <div class="pull-left" style="color:#fff">
            &copy; <?php echo date('Y'); ?> <?php echo $admintitle; ?> All Rights Reserved.
        </div>
    </div>
</div>
</body>
    ...
    $(function(){
        ...
        function checkForm(){
            if( '' == $.trim($('#username').val())){
                alert_msg.error('请输入登录用户名');
                return false;
            }

            if( '' == $.trim($('#password').val())){
                alert_msg.error('请输入登录密码');
                return false;
            }

            if (<?php echo $verify_type; ?> == 0) {
                if( false == check_result){
                    alert_msg.error('请拖动滑块到最右边');
                    return false;
                }
            }
        ...
        function complete(data){
            if(data.code==1){
                ...
                //刷新验证码
                $("#verifyimg").attr("src","<?php echo url('checkVerify'); ?>?tm='+Math.random()");

                $('button').removeClass('disabled').text('登　录');

                alert_msg.error(data.msg,data.url);
